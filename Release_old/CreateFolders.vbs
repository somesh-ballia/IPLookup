...
Function Execute()	
	...
	for iCount = 0 To NUMBER_OF_CPS-1
		...
		if True = PingSuccess(CPS_NAME(iCount)) Then
			...
			If True = CreateShareFolder (CPS_NAME(iCount),strUserLogin,USER_NAME,PASSWORD,strNewFolder,SHARE_NAME,SHARE_DESCRIPTION) Then
		...
		Else
			...
			objShell.Run "Log.vbs PROGRESS MACHINE_NOT_AVAILABLE_"&CPS_NAME(iCount),0,true
		End IF
	Next
