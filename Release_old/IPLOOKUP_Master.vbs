...
Function Main()
	...
	IF TRUE = ISValidOS(strRequiredOS) Then
		IF True = ReadConfigFile() Then
			...
			End IF			
		Else			
			objShell.Run "Log.vbs ERROR ERROR_READING_CONFIG_FILE",0,true
		End IF
	
	ELSE		
		objShell.Run "Log.vbs ERROR INVALID_OPERATING_SYSTEM_FOUND",0,true
	End IF
...
Function ReadConfigFile()
	...
	If "1" = ReadIni(strConfigFileName,"GENERAL","ENABLE_USER_FOLDER_CREATION") Then
		ENABLE_USER_FOLDER_CREATION = True
	Else
		ENABLE_USER_FOLDER_CREATION = False
	End IF
	objShell.Run "Log.vbs PROGRESS ENABLE_USER_FOLDER_CREATION_=_"&ENABLE_USER_FOLDER_CREATION,0,true

	If "1" = ReadIni(strConfigFileName,"GENERAL","ENABLE_FTP_SITE") Then
		ENABLE_FTP_SITE = True
	Else
		ENABLE_FTP_SITE = False
	End IF
	objShell.Run "Log.vbs PROGRESS ENABLE_FTP_SITE_=_"&ENABLE_FTP_SITE	,0,true
...
Function InstallFTPServer()
	...
	Wscript.echo "Executing IIS_Install.vbs"
...
Function CreateFolders()
	...
	Wscript.echo "Executing CreateFolders.vbs"
...
Function ManageFTPSite()
	...
	IF True = ENABLE_SETUP_INSTALL Then
		...
		Wscript.echo "ManageFTPSite.vbs"	
