...
Function Execute()
	...
	IF True = IsFTPRunning() Then
		...
		IF True = CreateBasePathDirectory() Then
				...
				if 1 = iSiteCount Then
					...
					IF strDefaultSiteName = strSiteName Then
						IF True = OVERWRITE_DEFAULT_SITE Then
							...
							ContinueExecution()
						Else
							objShell.Run "Log.vbs NOTIFICATION DEFAULT_FTP_SITE_EXISTING_OVERWRITE_DEFAULT_SITE_DISABLED ",0,true
							objShell.Run "Log.vbs PROGRESS DEFAULT_FTP_SITE_EXISTING_OVERWRITE_DEFAULT_SITE_DISABLED ",0,true
							objShell.Run "Log.vbs PROGRESS QUITING_SCRIPT ",0,true
						End If
					Else
			...
			End IF
		Else
			objShell.Run "Log.vbs NOTIFICATION CREATING_FOLDER_FAILURE_CreateBasePathDirectory_=_"&FTP_BASE_PATH,0,true
			objShell.Run "Log.vbs PROGRESS CREATING_FOLDER_FAILURE_CreateBasePathDirectory_=_"&FTP_BASE_PATH,0,true
		End IF
	Else
...
Function ContinueExecution()
	...
	retCreateFTPSite = CreateFTPSite(HOST_NAME,FTP_BASE_PATH,FTP_SITE_NAME,IP_ADDRESS,USER_NAME,PASSWORD)
			IF True = retCreateFTPSite Then
				...
				for icount = 0 To NUMBER_OF_CPS-1
					...
					IF False = FolderExist(strVirtualDirFolder) Then	
						...
						IF True = retCreateFolder Then 
							...
							strPhysicalDirPath = CPS_NAME(iCount)&"\"&SHARE_NAME
				...
				Next
			Else
				objShell.Run "Log.vbs NOTIFICATION CREATING_SITE_FAILURE_=_"&FTP_SITE_NAME,0,true
				objShell.Run "Log.vbs PROGRESS CREATING_SITE_FAILURE_=_"&FTP_SITE_NAME,0,true
			End IF
...
Function CreateVirtualDirectory(strFTPSiteName, strVirtualDirName, strPhysicalDirPath,strRemoteMachineName, strUserName, strPassword)
	...
	Set WshShell = WScript.CreateObject("WScript.Shell")
	command = "cmd /c iisftpdr /create "&strFTPSiteName&" "&strVirtualDirName&" "&strPhysicalDirPath&" /s "&strRemoteMachineName& " /u "&strUserName&" /p" &strPassword
